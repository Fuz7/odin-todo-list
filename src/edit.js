...
export function editProject(id) {
  const inputText = document.getElementById('projectEditTitle').value;
  let previousTitle;
  ...
  const sortedTask = myTask.map((item) => {
...
export function editMyTask(id, title, desc, date, project, milestone) {
  const selectedTask = myTask.find((item) => item.taskId === Number(id));
